    ...
    for chain in biopy_model:
        for res in chain:
            ...
            if not(is_aa(res,standard=True)):
                if res.resname == ligname.upper(): #.upper():
...
def process_one_pdbdir(dirs):
    all_pocket = []
    fix_pdb_lst = {'4e4n_onl.pdb': '0nl', '6hkn_g6e.pdb': 'g9e'}
    for d in os.listdir(dirs):
        if '.pdb' in d:
            try:
                if d.startswith('3MY0'):
                    print('debug')
                p = PDBParser()
                model = p.get_structure('0',os.path.join(dirs,d))[0] 
                if d not in fix_pdb_lst:
                    tmp_chain, liglist = extract_lig_recpt(model, re.search(r'_.*\.', d)[0][1:-1])
                else:
                    tmp_chain, liglist = extract_lig_recpt(model, fix_pdb_lst[d])
                # tmp_chain,liglist = extract_lig_recpt(model,re.search(r'_.*\.',d)[0][1:-1])
...
if __name__ == '__main__':
    process_one_pdbdir('/data/protein/SKData/KDBNet_pocket_pdb')
    #process_one_pdbdir('/home/jiayinjun/Chem_Aware_Interaction_Understanding/data/Validation_data_V2/ADRB2/PDB')
    # for f in tqdm(os.listdir('/home/jiayinjun/Chem_Aware_Interaction_Understanding/data/Validation_data_V2')):
    #     try:
    #         process_one_pdbdir(f'/home/jiayinjun/Chem_Aware_Interaction_Understanding/data/Validation_data_V2/{f}/PDB')
    #     except:
    #         pass